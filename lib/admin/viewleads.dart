...
class _ViewLeadsPageState extends State<ViewLeadsPage> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
            'http://13.201.213.5:4080/admin/fetchallemployeeleadregister'),
      );
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _filteredUserList =
              List<Map<String, dynamic>>.from(jsonData['userList']);
        });
      } else {
  ...
  void _showUserDetails(Map<String, dynamic> user) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            UserDetailsPage(user: user, isLoading: _isLoading),
      ),
    );
  }
                ...
                : _filteredUserList.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          Color color =
                              _avatarColors[_colorIndex % _avatarColors.length];
                          _colorIndex++;
                          return ListTile(
...
class UserDetailsPage extends StatelessWidget {
  ...
  const UserDetailsPage({Key? key, required this.user, required this.isLoading})
      : super(key: key);
      ...
      body: isLoading
          ...
          : ListView(
              ...
              children: [
                ...
                _buildDetailItem('Short Description',
                    user['leadShortDescription'].toString()),
                _buildDetailItem(
                    'Brief Description',
                    user['leadBriefDescription'] != null
                        ? user['leadBriefDescription'].toString()
                        : 'N/A'),
                _buildDetailItem(
                    'Meeting Time',
                    user['leadMeetingTime'] != null
                        ? user['leadMeetingTime'].toString()
                        : 'N/A'),
                _buildDetailItem(
                    'Meeting Date',
                    user['leadMeetingDate'] != null
                        ? user['leadMeetingDate'].toString()
                        : 'N/A'),
                _buildDetailItem(
                    'Possible Lead', user['possibleLead'].toString()),
